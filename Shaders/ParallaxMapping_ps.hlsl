...
float4 main(NormalMappingPixelShaderInput input) : SV_Target
{
	...
	const float4 textureColour = DiffuseSpecularMap.Sample(TexSampler, offsetTexCoord); // Use offset texture coordinate from parallax mapping
	const float3 diffuseMaterialColour = textureColour.rgb;
	const float specularMaterialColour = textureColour.a;

    float3 finalColour = resDiffuse * diffuseMaterialColour + 
                         resSpecular * specularMaterialColour;
