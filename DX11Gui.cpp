...
void CDX11Gui::Show(float& frameTime)
{
	...
	if (ImGui::BeginMainMenuBar())
	{
		...
		if (fileDialog.showFileDialog("OpenScene", imgui_addons::ImGuiFileBrowser::DialogMode::OPEN, ImVec2(700, 310), ".xml"))
		{
			...
			mScene = new IScene(mEngine,fileDialog.selected_fn);
		...
		if (ImGui::Begin("Add Object"), &addObj, ImGuiWindowFlags_NoDocking)
		{
			...
			if (ImGui::Button("Add"))
			{
				...
				case Simple:
					...
					{
						const auto newObj = new CGameObject(mEngine, mesh, name, tex);
				...
				case Pbr:
					...
					if (!mesh.empty())
					{
						const auto newObj = new CGameObject(mEngine, mesh, name);
		...
		if (ImGui::Button("Duplicate Selected Obj"))
		{
			...
			try
			{
				...
				else
				{
					...
					{
						const auto obj = new CGameObject(*mSelectedObj);
