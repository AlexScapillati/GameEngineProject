...
void CDX11Engine::InitDirect3D()
{
	...
	hr = D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, flags, 0, 0, D3D11_SDK_VERSION,
		&swapDesc, mSwapChain.GetAddressOf(), mD3DDevice.GetAddressOf(), nullptr, mD3DContext.GetAddressOf());
	if (FAILED(hr))
	{
...
CDX11Engine::CDX11Engine(HINSTANCE hInstance, int nCmdShow)
{
	...
	{
		// Create scene
		mMainScene = std::make_unique<CDX11Scene>(this, "Scene2.xml");
	}
	catch (const std::exception& e)
	{
		throw std::runtime_error(e.what());
...
void CDX11Engine::Resize(UINT x, UINT y)
{
	if (mSwapChain)
	{
		...
		if (x > 1 && y > 1)
		{
			...
			if (FAILED(hr))
			{
				throw std::runtime_error("Error resizing buffer");
			}
