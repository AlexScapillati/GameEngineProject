...
public:
	...
	CDX12GameObject(CDX12Engine* engine, std::string mesh, std::string name, std::string& diffuseMap, CVector3 position = { 0,0,0 }, CVector3 rotation = { 0,0,0 }, float scale = 1);
	...
	CDX12GameObject(CDX12Engine* engine, std::string id, std::string name, CVector3 position = { 0,0,0 }, CVector3 rotation = { 0,0,0 }, float scale = 1);
	...
	struct sAmbientMap
	{
		...
		void Init(CDX12Engine* engine);

		bool                                   enabled;
		Microsoft::WRL::ComPtr<ID3D12Resource> map;              // The actual texture stored on the GPU side (cubemap)
		Microsoft::WRL::ComPtr<ID3D12Resource> mapSRV;           // The texture in the shader resource view format, to send it to the shader
		Microsoft::WRL::ComPtr<ID3D12Resource> depthStencilMap;  // The depth stencil texture 
		Microsoft::WRL::ComPtr<ID3D12Resource> depthStencilView; // The depth stencil view to set it as the render target
		Microsoft::WRL::ComPtr<ID3D12Resource> RTV[6];           // The 6 different render targets, one for each face, to bind to the render target
	...
	ID3D12Resource* AmbientMapSRV() const;

	std::string MeshFileNames();
	ID3D12Resource* TextureSRV() const;
