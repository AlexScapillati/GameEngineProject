...
public:
	CSky(CDX11Engine* engine, std::string mesh,
		...
		float scale = 1)
		: CGameObject(engine, std::move(mesh), std::move(name), diffuse, position, rotation, scale)
	{
	...
	void Render(bool basicGeometry = false) override
	{
		...
		else
		{
			...
			mEngine->GetContext()->OMSetDepthStencilState(mEngine->mNoDepthBufferState, 0);

			//set the colour white for the sky (no tint)
			gPerModelConstants.objectColour = { 1, 1, 1 };

			// skyboxes point inwards
			mEngine->GetContext()->RSSetState(mEngine->mCullFrontState);
